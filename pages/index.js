...
export default function Home() {
  ...
  return <Layout title="PlainUtils">
    ...
    <section className="mb-12 overflow-hidden rounded-xl bg-gradient-to-br from-blue-600 to-violet-700 text-white p-6 sm:p-8"><div className="flex flex-col md:flex-row md:items-center justify-between gap-6"><div className="flex items-start gap-4"><span className="p-3 rounded-lg bg-white/15"><Workflow className="w-7 h-7"/></span><div><p className="text-sm font-semibold text-blue-100 uppercase tracking-wide">PlainUtils Workflows</p><h2 className="text-2xl sm:text-3xl font-bold mt-1">Start with a recipe. Make it yours.</h2><p className="mt-2 text-blue-100 max-w-2xl">Browse ready-made, private workflows for JSON, encoding, debugging, and text cleanup—or build your own pipeline from scratch.</p></div></div><div className="flex flex-wrap gap-2 shrink-0"><Link href="/workflows" className="inline-flex items-center justify-center gap-2 bg-white text-blue-700 px-5 py-3 rounded-md font-semibold hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-white">Browse recipes <ArrowRight className="w-4 h-4"/></Link><Link href="/workspace" className="inline-flex items-center justify-center px-5 py-3 rounded-md font-semibold border border-white/50 hover:bg-white/10">Build from scratch</Link></div></div></section>
