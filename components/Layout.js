...
import { BookMarked, Menu, Radar, Search, Workflow, X } from 'lucide-react';
  ...
  return (
    <nav aria-label="Tool navigation" className="space-y-1">
      {tools.map((tool) => {
        ...
        return <Link key={tool.href} href={tool.href} onClick={onNavigate} aria-current={router.pathname === tool.href ? 'page' : undefined} className={`tool-nav-link ${router.pathname === tool.href ? 'is-active' : ''}`}><span className="tool-nav-icon"><Icon className="w-4 h-4" /></span><span>{tool.title}</span></Link>;
      })}
    </nav>
  );
}
...
export default function Layout({ title, metaTitle, description = 'Fast, free and privacy-conscious browser tools for developers and everyday tasks.', keywords, canonical, hidePageHeader = false, children }) {
  ...
  const navLink = (href) => router.pathname === href ? 'main-nav-link is-active' : 'main-nav-link';
    ...
    <div className="site-shell min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 flex flex-col">
      ...
      <header className="site-header sticky top-0 z-40">
        <div className="px-4 sm:px-6 py-3 flex justify-between items-center max-w-7xl mx-auto">
          <div className="flex items-center gap-6"><Link href="/" className="brand-mark"><span className="brand-spark" aria-hidden="true"><i>P</i><i>U</i></span><span>Plain<span className="brand-accent">Utils</span></span></Link>
            <nav className="hidden md:flex items-center gap-1 text-sm font-semibold" aria-label="Main navigation"><Link href="/tools" className={navLink('/tools')}>Tools</Link><Link href="/detect" className={navLink('/detect')}><Radar/>Detect</Link><Link href="/workflows" className={navLink('/workflows')}><Workflow/>Workflows</Link><Link href="/library" className={navLink('/library')}><BookMarked/>Library</Link><Link href="/about" className={navLink('/about')}>About</Link></nav>
          </div>
          <div className="flex items-center gap-2"><Link href="/workspace" aria-label="Open pipeline workspace" className="workspace-pill"><Workflow /><span className="hidden lg:inline">Workspace</span></Link><button onClick={() => setPaletteOpen(true)} aria-label="Search tools, Command K" className="search-trigger"><Search /><span className="hidden lg:inline">Search</span><kbd className="hidden xl:inline">⌘K</kbd></button><DarkModeToggle /><button onClick={() => setMenuOpen(!menuOpen)} className="icon-button md:hidden" aria-expanded={menuOpen} aria-label="Toggle navigation">{menuOpen ? <X /> : <Menu />}</button></div>
        </div>
        {menuOpen && <nav className="md:hidden px-6 pb-4 grid gap-3" aria-label="Mobile navigation"><Link href="/tools">Tools</Link><Link href="/detect" className="flex items-center gap-2"><Radar className="w-4 h-4"/>Smart Detect</Link><Link href="/workflows" className="flex items-center gap-2"><Workflow className="w-4 h-4"/>Workflows</Link><Link href="/library" className="flex items-center gap-2"><BookMarked className="w-4 h-4"/>Library</Link><Link href="/about">About</Link><Link href="/privacy">Privacy</Link><Link href="/contact">Contact</Link></nav>}
      </header>
      ...
      <main id="main-content" className={`content-frame ${currentTool ? 'tool-layout' : ''}`}>
        {currentTool && <aside className="tool-sidebar hidden md:block"><p className="sidebar-label">Toolbox</p><ToolNavigation /></aside>}
        <div className="flex-1 min-w-0 content-surface">
      ...
      <footer className="site-footer"><div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between gap-3"><p><strong>PlainUtils</strong> · Small tools, properly made.</p><p>© {new Date().getFullYear()} · <Link href="/privacy">Privacy</Link> · <Link href="/contact">Contact</Link> · <a href="https://github.com/pingryte/plainutils" target="_blank" rel="noreferrer">GitHub</a> · <a href="https://coff.ee/pingryte" target="_blank" rel="noreferrer">Support</a></p></div></footer>
